...
parser.add_argument('--seed', type=int, default=1024, help='seed for random and torch')
parser.add_argument('--cvidx', type=int, default=0, help='index for cross validation data part')
...
def main():
    setseed(args.seed)
    print('Loading data...')
    train = pd.read_csv(args.data_dir + 'cv/train_cv_{}.tsv'.format(args.cvidx), sep = '\t')
    valid = pd.read_csv(args.data_dir + 'cv/valid_cv_{}.tsv'.format(args.cvidx), sep = '\t')
    ...
    if args.test:
        ckpt = torch.load(args.result_dir + '/best_checkpoint_cv{}_seed{}_bs{}_lr{}_ep{}.pth.tar'.format(args.cvidx, args.seed, args.batch_size, args.lr, args.epoch))
    ...
    for epoch in tqdm(range(args.epoch)):
        ...
        torch.save(ckpt_dict, args.result_dir + '/latest_checkpoint_cv{}.pth.tar'.format(args.cvidx))

        if macro_f1 > best_f1:
            best_f1 = macro_f1
            torch.save(ckpt_dict, args.result_dir + '/best_checkpoint_cv{}_seed{}_bs{}_lr{}_ep{}.pth.tar'.format(args.cvidx, args.seed, args.batch_size, args.lr, args.epoch))
